...
void Adafruit_LSM303_Accel_Unified::setTap(int mode, int threshold, int timeLimit, int timeLatency, int timeWindow, int tapCfg) {
  bool tapInterruptEnable = true;
  
  if (mode == 0 & tapCfg == 0) {
    tapInterruptEnable = false;
    tapCfg = 0;
  }

  if (mode == 1) {
    tapCfg = 0x15; //All Axes on with single tap
  } else if (mode == 2) {
    tapCfg = 0x2A; //All Axes on with double tap
  }

  Adafruit_BusIO_Register ctrl_3 =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_CTRL_REG3_A, 1);
  Adafruit_BusIO_RegisterBits active_bit =
      Adafruit_BusIO_RegisterBits(&ctrl_3, 1, 1);
  active_bit.write(tapInterruptEnable);

  Adafruit_BusIO_Register tap_cfg =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_CLICK_CFG_A, 1);

  tap_cfg.write(tapCfg);

  if (!tapInterruptEnable) return;

  Adafruit_BusIO_Register tap_threshold =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_CLICK_THS_A, 1);
  Adafruit_BusIO_Register tap_limit =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_TIME_LIMIT_A, 1);
  Adafruit_BusIO_Register tap_latency =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_TIME_LATENCY_A, 1);
  Adafruit_BusIO_Register tap_window =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_TIME_WINDOW_A, 1);

  tap_threshold.write(threshold);
  tap_limit.write(timeLimit);
  tap_latency.write(timeLatency);
  tap_window.write(timeWindow);
}

bool Adafruit_LSM303_Accel_Unified::tapped() {
  int tapSource;

  Adafruit_BusIO_Register tap_source =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_CLICK_SRC_A, 1);
  
  tapSource = tap_source.read();

  return tapSource & 0b1000000 > 0;
}

int Adafruit_LSM303_Accel_Unified::getTapped() {
  Adafruit_BusIO_Register tap_source =
      Adafruit_BusIO_Register(i2c_dev, LSM303_REGISTER_ACCEL_CLICK_SRC_A, 1);
  
  return tap_source.read();
}
